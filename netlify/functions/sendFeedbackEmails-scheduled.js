// netlify/functions/sendFeedbackEmails-scheduled.js

const nodemailer = require("nodemailer");
const axios = require("axios");

// Netlify Scheduled Function config (CommonJS)
exports.config = {
...
exports.handler = async () => {
  ...
  try {
    ...
    const testers =
      res.data && Array.isArray(res.data.testers) ? res.data.testers : [];
    ...
    for (const tester of testers) {
      ...
      try {
        ...
        console.log(
          `✓ Email sent to ${email} (Message ID: ${emailResult.messageId})`
        );
        ...
        try {
          ...
          console.error(
            `WARNING: Email sent but failed to mark in sheet for ${email}:`,
            markError.message
          );
          // Email was sent successfully, still count success
          successCount++;
        ...
        // Don't mark as sent if email failed - it will retry next run
      }
    }
